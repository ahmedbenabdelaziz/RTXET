...
class admiens extends StatefulWidget {
  final String name;
  final String userId;
  final String token;
  ...
  @override
  State<admiens> createState() => _admiensState();
}

class _admiensState extends State<admiens> {
 ...
 late int chatidd=15;
  Future<void> getChatid()async{
    ...
    http.Response response = await http.get(Uri.parse('http://192.168.1.13/ISIMM_eCampus/public/api/chatAdmin'),
        headers: headers);
  ...
  Future<Map<String, dynamic>> getMsgEtudient(token,idchat)async{
    ...
    http.Response response = await http.get(Uri.parse('http://192.168.1.13/ISIMM_eCampus/public/api/chat_messages_enseignant/${chatidd}'),
        headers: headers);
    ...
    return Text("${snapshot.error}");
      ...
      return ListView.builder(
        ...
        itemBuilder: (BuildContext context, int index) {
          if (index < msg.length) {
            if (i <= msg.length) {
              if (msg[index]['sender_id'] ==msg[i]['enseignant_id'] ) {
                i++;
                print(i);
