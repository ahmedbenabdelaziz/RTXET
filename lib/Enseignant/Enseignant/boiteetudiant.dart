...
class admietud extends StatefulWidget {
  final String name;
  final String userId;
  final String token;
  ...
  @override
  State<admietud> createState() => _admietudState();
}

class _admietudState extends State<admietud> {
  ...
  Future<void> getChatid()async{
    ...
    http.Response response = await http.get(Uri.parse('http://192.168.1.13/ISIMM_eCampus/public/api/chatAdmine'),
        headers: headers);
    var data = jsonDecode(response.body)['chatAdmin'] as Map<String,dynamic>;
  ...
  Future<Map<String, dynamic>> getMsgEtudient(token,idchat)async{
    ...
    http.Response response = await http.get(Uri.parse('http://192.168.1.13/ISIMM_eCampus/public/api/chat_messages_etudiant/${chatidd}'),
        headers: headers);
                ...
                children: [
                  Container(
                    ...
                    child: Row(
                      ...
                      children: [
                        Image.asset('lib/Enseignant/imgprof/Capture dՎcran 2023-05-28 190421.png',height: 40,),
                        SizedBox(width: 10),
                        Column(
                          ...
                          if (!snapshot.hasData) {
                            ...
                            return ListView.builder(
                              ...
                              itemBuilder: (BuildContext context, int index) {
                                if (index < msg.length) {
                                  if (i <= msg.length) {
                                    if (msg[index]['sender_id'] ==msg[i]['student_id'] ) {
                                      i++;
                                      print(i);
