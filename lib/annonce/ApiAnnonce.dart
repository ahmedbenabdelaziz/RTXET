...
class ApiAnnonce {
  ...
  Future<Map<String, dynamic>> getAnnonce(token) async {
  ...
  Future<void> postAnnonce(String post, image, token) async {
    var request = http.MultipartRequest(
      'POST',
      Uri.parse('http://192.168.1.13/ISIMM_eCampus/public/api/enseignant/ajouter_annonce'),
    );
    ...
    if (response.statusCode == 200) {
      print(jsonResponse);
      getAnnonce(token); // Appel de la méthode getAnnonce() pour mettre à jour les données
    } else {
      print(jsonResponse);
    }
  }
  ...
  Future<void> postAnnonceEtudiant(String post, image, token) async {
    print("333");
    print(token);
    var request = http.MultipartRequest(
      'POST',
